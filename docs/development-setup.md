...
変更した振る舞いと影響範囲に応じて、必要な検証を選びます。
軽微で振る舞いを変えない変更に、テスト追加や全体テストを一律に要求しません。
対象を絞ったテストで十分ならそれを使い、Android の実動作を必要とする検証だけを端末・エミュレーターで実行します。
必要な検証が成功した後は、新たな変更・失敗・未解決の懸念がない限り、検証を拡大・反復しません。
実行した検証と結果、必要だが実施できなかった検証と理由を報告します。
